...
1. Set `Test.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../Test.Infrastructure/Test.Infrastructure.csproj -s Test.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../Test.Infrastructure/Test.Infrastructure.csproj -s Test.Web.csproj
```
