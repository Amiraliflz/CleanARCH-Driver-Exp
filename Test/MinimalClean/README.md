...
# Run the application
dotnet run --project src/MinimalTest.Web

# Or run with Aspire (if using)
dotnet run --project src/MinimalTest.AspireHost
```
...
```powershell
dotnet ef database update -c AppDbContext -p src/MinimalTest.Web -s src/MinimalTest.Web
dotnet run --project src/MinimalTest.Web
```
